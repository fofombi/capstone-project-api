...
// { patient: { title: '', text: 'foo' } } -> { example: { text: 'foo' } }
...
router.post('/patients', requireToken, (req, res, next) => {
  ...
  patient.create(req.body.patient)
    ...
    // the error handler needs the error patient and the `res` object so that it
    // can send an error patient back to the client
    .catch(next)
})
